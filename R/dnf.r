...
dnf.moments <- function(df1,df2,ncp1,ncp2,order.max=3) {
	orders = 1:order.max
	log.numr <- chisq.moms(df=df1,ncp=ncp1,orders=orders,log=TRUE) - orders*log(df1)
	log.deno <- chisq.moms(df=df2,ncp=ncp2,orders=-orders,log=TRUE) + orders*log(df2)
...
#
# where the chi^2 are independent chi-square
dnf.cumuls <- function(df1,df2,ncp1,ncp2,order.max=3) {
	kappa <- moment2cumulant(dnf.moments(df1,df2,ncp1,ncp2,order.max))
	return(kappa)
}
...
ddnf <- function(x, df1, df2, ncp1, ncp2, log = FALSE, order.max=6) {
	kappa <- dnf.cumuls(df1,df2,ncp1,ncp2,order.max=order.max)
