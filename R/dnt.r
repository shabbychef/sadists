...
dnt.moments <- function(df,ncp1,ncp2,order.max=3) {
	orders = 1:order.max
	numr <- norm.moms(mu=ncp1,sigma=1,order.max=order.max)
	deno <- exp(chisq.moms(df=df,ncp=ncp2,orders=-orders/2.0,log=TRUE) + (orders/2.0)*log(df))
	mus <- numr * deno
	return(mus)
}
...
#
# where the Z is independent of the chi-square.
dnt.cumuls <- function(df,ncp1,ncp2,order.max=3) {
	kappa <- moment2cumulant(dnt.moments(df,ncp1,ncp2,order.max))
	return(kappa)
}
...
ddnt <- function(x,df,ncp1,ncp2,log=FALSE, order.max=6) {
	kappa <- dnt.cumuls(df,ncp1,ncp2,order.max=order.max)
