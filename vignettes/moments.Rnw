...
Note that this does not seem to match the equation found by 
Moser, even in the simple case $\nccp=0$. \cite{moser2004}
To check this formula, a simple Monte Carlo experiment is
reassuring:

<<'mc_check',echo=TRUE,eval=TRUE,results='as-is'>>=
require(PDQutils)
