...
several chi-square random variables. The \kth{k} raw moment of
a chi-square random variable with \df degrees of freedom is
$2^k \fracc{\GAM{k + \df/2}}{\GAM{\df/2}}$. The raw moments of the
...
the density. For even a small number of factors, the Edgeworth expansion
...
The derivative of the log of the Gamma function is the `psi' function,
$\psif{\cdot}$, while its derivatives are the `polygamma' functions,
$\polygam[n]{\cdot}$.  \cite[6.3, 6.4]{abramowitz_stegun}
