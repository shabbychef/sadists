...
%\VignetteIndexEntry{Using the sadists Package}
%\VignetteKeyword{Finance}
%\VignetteKeyword{sadists}
...
\title{Using the \pkg{sadists} package}
...
Example computations via the \sadists package are illustrated.
\end{abstract}%UNFOLD
