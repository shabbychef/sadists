...
This is not a common distribution. 
Its moments can be computed, so the `pdq' functions can be
approximated by classical expansions. This distribution
displays the limitations of the Edgeworth and Cornish Fisher
expansions, displaying poor convergence properties
