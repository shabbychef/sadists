...
opts_chunk$set(fig.width=5.0,fig.height=3.0,out.width="5in",out.height="3in",dpi=300)
...
which does the heavy lifting once the cumulants have been computed.

It should be noted that the distributions provided by \sadists do \emph{not}
...
\section{Sum of (non-central) chi-squares}%FOLDUP
...
Y = \sum_i w_i X_i
$$
follows a weighted sum of non-central chi-squares distribution. This is not a 
common distribution. However, its cumulants can be
easily computed, so the `PDQ' functions can be approximated by classical
expansions. Moreover, its CDF and quantile functions can be used to compute
those of the doubly non-central F.
...
@
%UNFOLD
\section{Sum of (non-central) chis}%FOLDUP
...
$\nu_i$ are the degrees of freedom. Let $w_i$ be given constants. Then
$$
Y = \sum_i w_i \sqrt{X_i}
$$
follows a weighted sum of non-central chis distribution. This is not a 
common distribution. 
Adding a normal to the sum above gives an upsilon distribution, see 
...
Generalizing the random variables from \secref{sumchisq} and \secref{sumchi},
we can take sums of chi-squares to powers.
Let $X_i \sim \chi^2_{\nu_i}\left(\delta_i\right)$ be independent non-central
chi-square variates, where $\delta_i$ are the non-centrality parameters and
...
<<'sumchipow',echo=TRUE,eval=TRUE,fig.cap="Confirming the dpqr functions of the sum of chi-squares to a power distribution.">>=
require(sadists)
wts <- c(-1,1,3,-3)
df <- c(100,200,100,50)
ncp <- c(0,1,0.5,2)
