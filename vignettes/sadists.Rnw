...
\section{Sum of (non-central) Chi-squares}
...
<<'sumchisq',echo=TRUE,eval=TRUE>>=
require(sadists)
wts <- c(1,-3,4)
df <- c(100,20,10)
ncp <- c(5,3,1)
testf(list(d=dsumchisq,p=psumchisq,q=qsumchisq,r=rsumchisq),nobs=8192,wts,df,ncp)
@

\section{Sum of (central) Chi}
...
<<'sumchi',echo=TRUE,eval=TRUE>>=
require(sadists)
wts <- c(-3,2,5,-4,1)
df <- c(30,50,100,20,10)
testf(list(d=dsumchi,p=psumchi,q=qsumchi,r=rsumchi),nobs=8192,wts,df)
@
...
\section{Lambda prime distribution}

A Lambda prime random variable is the sum of a standard
...
\section{Upsilon distribution}

An upsilon random variable is the sum of a standard
normal and the weigted sum of several indpendent central chis.

<<'upsilon',echo=TRUE,eval=TRUE>>=
require(sadists)
df <- c(30,50,100,20,10)
ts <- c(-3,2,5,-4,1)
testf(list(d=dupsilon,p=pupsilon,q=qupsilon,r=rupsilon),nobs=8192,df,ts)
@
...
<<'nakagami',echo=TRUE,eval=TRUE>>=
require(sadists)
m <- 120
Omega <- 4
testf(list(d=dnakagami,p=pnakagami,q=qnakagami,r=rnakagami),nobs=8192,m,Omega)
@

\section{Doubly non-central t distribution}
...
\section{Doubly non-central F distribution}
