...
@
%UNFOLD
\section{Sum of (central) chi}%FOLDUP
...
$$
follows a weighted sum of central chis distribution. Ideally one would like to
capture the distribution for the case where the chis are non-central, but the
cumulants are not apparently known. 
...
when $b=0, a = 1, \nu_2 = \infty$.
\end{itemize}
This distribution is not yet implemented in \sadists.
...
(\secref{upsilon_distribution}). It is currently available as a special
case of the Upsilon distribution.
...
\cite{Lecoutre1999,PoitLecoutre2010}
%UNFOLD
...
@
%UNFOLD
\section{Nakagami distribution}%FOLDUP
